...
rule_base_recalibration_first= """rule base_recalibration_first:
  ...
  shell: "java -Xmx4g -jar %s -T BaseRecalibrator -I {input.bam} -R {input.ref} -knownSites {input.known} -knownSites {input.known_sites} -o {output}"\n""" %GATK_JAR
...
rule_base_recalibration_second= """rule base_recalibration_second:
  ...
  shell: "java -Xmx4g -jar %s -T BaseRecalibrator -I {input.bam} -R {input.ref} -knownSites {input.known} -knownSites {input.known_sites} -BQSR {input.bqsr} -o {output}"\n""" %GATK_JAR
...
rule_VariantAnnotator= """rule VariantAnnotator:
  ...
  shell: "java -Xmx4g -jar %s -T VariantAnnotator -R {input.ref} --variant {input.vcf} -A StrandOddsRatio -A FisherStrand -o {output}"\n""" %GATK_JAR
...
rule_apply_recalibration= """rule apply_recalibration:
  shell: "java -Xmx4g -jar %s -T ApplyRecalibration -R {input.ref} -input {input.vcf} -mode SNP --ts_filter_level 99.0 -recalFile {input.recalfile} -tranchesFile {input.tranches_file} -o {output}"\n""" %GATK_JAR
...
rule_extract_moderate_to_csv= """rule extract_moderate_to_csv:
  ...
  shell: "java -Xmx4g -jar %s extractFields -s ',' -e '.' {input.vcf} 'CHROM' 'POS' 'REF' 'ALT' 'ID' 'DP' 'EFF[*].IMPACT' 'EFF[*].GENE' 'dbNSFP_SIFT_pred' 'dbNSFP_Polyphen2_HVAR_pred' 'dbNSFP_MutationTaster_pred' 'dbNSFP_1000Gp1_ASN_AF' > {output}"\n""" %SNPSIFT_JAR
